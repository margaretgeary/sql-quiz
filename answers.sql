...
SELECT orders.id, status, order_total FROM orders JOIN customers ON (customer_id=customers.id) WHERE customers.email = 'pclark74@gmail.com';
...
SELECT common_name, melon_type, quantity, unit_price, total_price FROM melons JOIN order_items ON (melons.id=order_items.melon_id) WHERE order_items.order_id = 2725;
